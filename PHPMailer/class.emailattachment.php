...
class ReadAttachment
{
	...
	function getdata($host,$login,$password,$savedirpath,$delete_emails=false) {
		...
		$emails = imap_search($mbox,'SUBJECT "300"');
		print_r($emails);
		// echo 'imap_num_msg($mbox)imap_num_msg($mbox)'.imap_num_msg($emails);
	//for ($jk = 1; $jk <= imap_num_msg($mbox); $jk++) {
		
		$jk=imap_num_msg($mbox);
...
function getdataLast($host,$login,$password,$savedirpath,$delete_emails=false) {
		// make sure save path has trailing slash (/)
		$savedirpath = str_replace('\\', '/', $savedirpath);
		if (substr($savedirpath, strlen($savedirpath) - 1) != '/') {
			$savedirpath .= '/';
