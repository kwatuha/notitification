...
function createMsgTrack($source){
	$fielddata=fillPrimaryData('sms_msgcenterdefault',1);
	$centerId=$fielddata['center'];
	$sourcedata=fillPrimaryData('sms_msgsource',$source);
	return $sourcedata['message_source'].$fielddata['center'];
}

function executeRemoteQuery($querySQL,$remoteID,$lastId){
$origMessage=getQueryRemoteTemplate($remoteID);
$dataCols=getCstTempCols($origMessage);
$messageId=createMsgTrack(3);
//  $messageId=createMsgTrack(3);
	$msgback="function savedData(){
								...
								savedData();";
					$errorMessage='{success:true, savemsg:'.json_encode($msgback).'}';


	$alertQueryResults=mysql_query($querySQL) or die($errorMessage);
	$cntreg_stmnt=mysql_num_rows($alertQueryResults); 
	if($cntreg_stmnt<=0){
		return 'nodata';
	}
	...
	if($lastId){
			while($rows=mysql_fetch_array($alertQueryResults)){
						$smsMSG=customizeRDMsg($dataCols,$origMessage,$rows);
						$phoneNumber=$rows['Prev_read'];	
						//  echo "insertMsg('sms_msgqueue',$smsMSG,$phoneNumber,'remote source');"	;			
						insertMsg('sms_msgqueue',$smsMSG,$phoneNumber,$messageId.'_'.$lastId);
		}
	}
...
function addRemoteItemsToQueue($alertQueryResults,$lastId){
$messageId=createMsgTrack(3);

print_r($alertQueryResults);
//  while($rows=mysql_fetch_array($alertQueryResults)){
// 		             $smsMSG=customizeRDMsg($dataCols,$origMessage,$rows);
// 					 $phoneNumber=$rows['Prev_read'];	
// 					//  echo "insertMsg('sms_msgqueue',$smsMSG,$phoneNumber,'remote source');"	;			
...
function customizeRDMsg($columDefinition,$message,$dataRow){
...
function getQueryRemoteTemplate($remoteID){
  $query_Rcd_getmenus= " select  message from sms_datasourcetemp 
  inner join sms_smsmsgcust on sms_smsmsgcust.smsmsgcust_id= sms_datasourcetemp.smsmsgcust_id  
  where datasourcetemp_id='$remoteID'  and sms_datasourcetemp.voided=0";
	$alertQueryResults=mysql_query($query_Rcd_getmenus);
	$data="";

  while($table_autofillrows=mysql_fetch_array($alertQueryResults)){
					 $data=$table_autofillrows['message'];
...
function processRemoteDBQuery($remoteDataID,$paramData){
	$query=getRemoteQuery($remoteDataID);
	$data =json_decode($paramData, false);
	$paramValues="";
        if (is_array($data->data)) {
           $i=1;
            foreach ($data->data as $value) {
				$valParam=$value->param;
				if(($valParam=='monthfilter') || ($valParam=='yearfilter')){					
					if($valParam=='yearfilter') {
						$query=str_replace('{'.$valParam.'}',$_POST['yearfilter_id'],$query);
						}
					if($valParam=='monthfilter') {
						$query=str_replace('{'.$valParam.'}',$_POST['monthfilter_id'],$query);
					}

				}else{
					$query=str_replace('{'.$valParam.'}',$_POST['param'.$i],$query);
					$i++;
...
function getQueryRemoteParams($remoteID){
  $query_Rcd_getmenus= " select parameters from sms_datasourcetemp where datasourcetemp_id='$remoteID'";
	$alertQueryResults=mysql_query($query_Rcd_getmenus);
	$data="";

  while($table_autofillrows=mysql_fetch_array($alertQueryResults)){
					 $data=$table_autofillrows['parameters'];
					 
					 }
           return $data;
}

function getRemoteQuery($remoteID){
  $query_Rcd_getmenus= " select query from sms_datasourcetemp where datasourcetemp_id='$remoteID'";
	$alertQueryResults=mysql_query($query_Rcd_getmenus);
	$data="";

  while($table_autofillrows=mysql_fetch_array($alertQueryResults)){
					 $data=$table_autofillrows['query'];
					 
					 }
           return $data;
}

function processRemoteDBParams($paramData){
	$data =json_decode($paramData, false);
	$paramValues="";
	$comma=',';
        if (is_array($data->data)) {
           $i=1;$allcnt=0;
            foreach ($data->data as $value) {
				...
				if(($valParam=='monthfilter') || ($valParam=='yearfilter')){
					...
					if($valParam=='yearfilter') {
						$paramValues.='{"'.$valParam.'":"'.$_POST['yearfilter_id'].'"}'.$comma;
						}
					if($valParam=='monthfilter') {
						$paramValues.='{"'.$valParam.'":"'.$_POST['monthfilter_id'].'"}'.$comma; // $valParam."::".$_POST['monthfilter_id'];
					}

				}else{
					$paramValues.='{"'.$valParam.'":"'.$_POST['param'.$i].'"}'.$comma;
					// $paramValues.=$valParam.":".'param='.$_POST['param1'];
					$i++;
				}
...
function markPrinted($queue_id){


$SQlUpdate="update medicallab_resultreview  set sys_track ='Printed' where ucase(is_approved) like 'APPROVED' and queue_id='$queue_id'" ;
$voided=0;
$comment='';
$date_created=date('Y-m-d');
$created_by=trim($_SESSION['my_useridloggened']);
$uuid=gen_uuid();
$INSERTsql="INSERT INTO medicallab_resultprinthist(
queue_id,
comment,
created_by,
voided,
...
$results_stmt1=mysql_query($INSERTsql) or die($INSERTsql);                                          

$lastid=mysql_insert_id();
if($lastid){
$results_stmt1=mysql_query($SQlUpdate) or die($SQlUpdate); 
}
////
...
$sqlupdateVoidedeColumns="update $table set voided=0 where voided is Null or voided=''";
...
$rf_no_attachmentore="{xtype:'fieldset',
		   title:'More Attachments xx',
		   collapsible:true,
		   id:'mysuser',
		   items:[{
            xtype: 'button',
...
?><?php
function reviseSQLToDynamic($currentTable,$vardynami,$dynamiccolmnval,$isSearch){
						...
						$selectdynamic='';
						$innerJoin='';
						$hasWheres=explode('where',$_SESSION[$currentTable."_SearchSQL"]);
						if(	sizeof($hasWheres)<=1 && !$isSearch)
						$strSqlvoided=" where $currentTable.voided=0 ";
...
//$searchStr=$_POST['username'];
$actualstmt="Select ucase(username) from $tablename where username='".strtoupper($searchStr)."'";
